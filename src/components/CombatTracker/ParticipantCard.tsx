...
import {
  IconSword, IconChevronDown, IconFlask, IconSkull, IconCheck, IconX, IconSparkles
...
import { useCharacterSheet, useResourceTracker } from '@/hooks/useCombat'
import type { ParticipantWithStatus, Condition } from '@/types'
import HpBar from '@/components/shared/HpBar'
import { notifications } from '@mantine/notifications'
import ResourcePanel, { ResourceSummaryBadge } from './ResourcePanel'
...
function DeathSavesPanel({ participant }: { participant: ParticipantWithStatus }) {
  ...
  async function handleReset() { await resetDeathSaves(participant.id) }

  return (
    <Box style={{ background: 'var(--mantine-color-dark-6)', borderRadius: 'var(--mantine-radius-sm)', padding: '8px 10px', border: '1px solid var(--mantine-color-dark-4)' }}>
      <Group justify="space-between" mb={6}>
        ...
        <Tooltip label="Resetear tiradas">
          <ActionIcon size="xs" variant="subtle" color="gray" onClick={handleReset}><IconX size={10} /></ActionIcon>
        </Tooltip>
      </Group>
      <Group gap="xl">
          ...
          <Group gap={4}>
            {[0, 1, 2].map(i => (
              <Tooltip key={i} label={i < successes ? 'Click para resetear' : 'Registrar éxito'}>
                <ActionIcon size="sm" radius="xl" variant={i < successes ? 'filled' : 'outline'} color="green" onClick={i < successes ? handleReset : handleSuccess}>
                  <IconCheck size={12} />
                </ActionIcon>
              </Tooltip>
            ))}
          ...
          <Group gap={4}>
            {[0, 1, 2].map(i => (
              <Tooltip key={i} label={i < failures ? 'Click para resetear' : 'Registrar fallo'}>
                <ActionIcon size="sm" radius="xl" variant={i < failures ? 'filled' : 'outline'} color="red" onClick={i < failures ? handleReset : handleFailure}>
                  <IconX size={12} />
                </ActionIcon>
              </Tooltip>
            ))}
...
function ConditionsPanel({ participant }: { participant: ParticipantWithStatus }) {
  return (
    <Stack gap="xs">
      <Text size="xs" fw={600} c="dimmed" tt="uppercase" style={{ letterSpacing: '0.04em' }}>Estados</Text>
      <Group gap="xs">
          ...
          return (
            <Tooltip key={value} label={description} multiline w={230} position="top">
              <Badge
                ...
                size="sm"
                style={{ cursor: 'pointer', textTransform: 'capitalize', opacity: active ? 1 : 0.55, transition: 'all 0.15s ease' }}
                onClick={() => toggleCondition(participant, value)}
              >
                {label}
              </Badge>
            </Tooltip>
...
type ExpandedPanel = 'details' | 'conditions' | 'resources' | null
...
export default function ParticipantCard({ participant, isDead = false }: ParticipantCardProps) {
  ...
  // Load character sheet if participant was added from library
  const sheet  = useCharacterSheet(participant.characterSheetId)
  const tracker = useResourceTracker(participant.characterSheetId ? participant.id : null)

  // Show resource button for adventurers/NPCs with a sheet that has class OR racial traits
  const hasResources = !isDead
    && participant.type === 'adventurer'
    && !!participant.characterSheetId
    && !!sheet
    && (!!sheet.className || (sheet.racialTraits?.length ?? 0) > 0)
    && (!!sheet.spellSlots || (sheet.limitedFeatures?.length ?? 0) > 0 || (sheet.racialTraits?.length ?? 0) > 0)

  const isAdventurer = participant.type === 'adventurer'
  const isDown       = participant.isDown

  const currentPanel: ExpandedPanel = (() => {
    if (expandedParticipantId === `${participant.id}:details`)    return 'details'
    if (expandedParticipantId === `${participant.id}:conditions`) return 'conditions'
    if (expandedParticipantId === `${participant.id}:resources`)  return 'resources'
    return null
  })()
  ...
  return (
    <Card withBorder padding="sm" radius="md" style={cardStyle}>
      <Stack gap="xs">
          ...
          <Stack gap={2} style={{ flex: 1, minWidth: 0 }}>
            <Group gap="xs" wrap="nowrap">
              {participant.isActive && (
                <Badge color={isAdventurer ? 'blue' : 'red'} variant="filled" size="xs" style={{ flexShrink: 0 }}>
                  ▶ Turno
                </Badge>
              )}
              <Text fw={participant.isActive ? 700 : 500} size="sm" truncate title={participant.name}>
              ...
              {participant.conditions.slice(0, 3).map(c => {
                ...
                return <Badge key={c} color="orange" variant="light" size="xs" style={{ flexShrink: 0 }}>{cDef?.label ?? c}</Badge>
              })}
              {participant.conditions.length > 3 && (
                <Badge color="orange" variant="light" size="xs" style={{ flexShrink: 0 }}>+{participant.conditions.length - 3}</Badge>
              )}
            </Group>
            ...
            <Group gap="md">
              ...
              {!isDead && !isDown && (
                <Text size="xs" c={hpColor(participant.hpPercentage)} fw={500}>
                  ...
                  {participant.temporaryHp > 0 && <Text span c="cyan"> +{participant.temporaryHp}</Text>}
                </Text>
              )}
              {isDown && <Text size="xs" c="red" fw={600}>⚠ Abatido — 0 HP</Text>}
            </Group>
          </Stack>
          ...
          <Group gap={4} wrap="nowrap" style={{ flexShrink: 0 }}>
            {!isDead && (
              <Tooltip label="Aplicar daño / curar" position="left">
                <ActionIcon variant={participant.isActive ? 'filled' : 'light'} color="red" onClick={() => openDamageModal(participant)} aria-label="Daño/curar">
                  <IconSword size={16} />
                </ActionIcon>
              </Tooltip>
            )}
            ...
            {!isDead && (
              <Tooltip label="Estados / condiciones" position="left">
                <ActionIcon variant={currentPanel === 'conditions' ? 'filled' : 'light'} color="orange" onClick={() => togglePanel('conditions')} aria-label="Estados">
                  <IconFlask size={16} />
                </ActionIcon>
              </Tooltip>
            )}

            {/* Resources button — only for adventurers with class from library */}
            {hasResources && (
              <Tooltip label="Hechizos y habilidades" position="left">
                <ActionIcon
                  variant={currentPanel === 'resources' ? 'filled' : 'light'}
                  color="violet"
            ...
            <Tooltip label={isDown ? 'Salvaciones contra la muerte' : 'Ver detalles'} position="left">
              <ActionIcon
                ...
                style={{ animation: isDown && currentPanel !== 'details' ? 'pulse 1.5s infinite' : 'none' }}
              >
                {isDown
                  ? <IconSkull size={16} />
                  : <IconChevronDown size={16} style={{ transform: currentPanel === 'details' ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
                }
              </ActionIcon>
            </Tooltip>
          </Group>
        ...
        {/* ── HP bar ── */}
        {!isDead && <HpBar current={isDown ? 0 : participant.currentHp} max={participant.maxHp} tempHp={participant.temporaryHp} />}

        {/* ── Panel: details / death saves ── */}
        {currentPanel === 'details' && (
          <Stack gap="sm" pt="xs" style={{ borderTop: '1px solid var(--mantine-color-default-border)' }}>
            ...
            {/* Resource summary when there are resources to show */}
            {sheet && sheet.className && tracker && (
              <ResourceSummaryBadge sheet={sheet} tracker={tracker} />
            )}
        ...
        {/* ── Panel: resources (spells + features + racial traits) ── */}
        {currentPanel === 'resources' && sheet && (
          <Box pt="xs" style={{ borderTop: '1px solid var(--mantine-color-default-border)' }}>
            <ResourcePanel
